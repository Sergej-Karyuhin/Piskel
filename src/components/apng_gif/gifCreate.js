...
import { framesArray } from '../frames/frames';

import API from './gifshot';

export default class GIFCreate {
  constructor() {
    this.size = undefined;
    this.gifDownloadButton = document.querySelector('.download__gif');
  }
  ...
  makeGIF() {
    ...
    if (!this.gifDownloadButton.getAttribute('download-data')) {
      ...
      API.createGIF({
        ...
        if (!obj.error) {
          ...
          const gifDownloadButton = document.querySelector('.download__gif');
          gifDownloadButton.href = image;
          gifDownloadButton.setAttribute('download', 'animated-image-gif');
          gifDownloadButton.setAttribute('download-data', 'loadFile');
        }
      });
      ...
      const gifDownloadButton = document.querySelector('.download__gif');
      gifDownloadButton.removeAttribute('download-data');
      setTimeout(() => {
