...
export default class Bucket {
  constructor() {
    ...
    this.startB = 0;
    this.startA = 1;
    this.colorsLayer = this.ctx.getImageData(0, 0, this.canvasWidth, this.canvasHeight);
  ...
  matchStartColor(pixelPos) {
    const r = this.colorsLayer.data[pixelPos];
    const g = this.colorsLayer.data[pixelPos + 1];
    const b = this.colorsLayer.data[pixelPos + 2];
  ...
  colorPixel(pixelPos) {
    ...
    this.colorsLayer.data[pixelPos] = fillColorR;
    this.colorsLayer.data[pixelPos + 1] = fillColorG;
    this.colorsLayer.data[pixelPos + 2] = fillColorB;
    this.colorsLayer.data[pixelPos + 3] = 255;
  }
  ...
  paint(e) {
    ...
    this.ctx.putImageData(this.colorsLayer, 0, 0);
  }
  ...
  init() {
    ...
    this.canvasBackground.addEventListener('contextmenu', Bucket.canselDefaultRightClick.bind(this));

    // Custom cursor
    const canvasContainer = document.querySelector('#canvas__wrapper');
    canvasContainer.style.cursor = 'url(\'https://www.piskelapp.com/p/img/cursors/paint-bucket.png\') 16 16, auto';
  }
}
